    ...
    return (
        <div className="p-5 md:flex md:flex-col md:ml-12">
            <div className='ml-10 md:ml-[80px]'>
                <div>
                    <p className="mb-1 mt-5 text-left text-xl"> IDEAS </p>
                </div>
                <div>
                    <h1 className="text-left mb-7 leading-tight text-[48px]">Here is what I want to build</h1>
                </div>
                <div>
            ...
            <div className="bg-[#f6edee] flex flex-col md:flex-row md:justify-between md:items-center mx-4 md:mx-24 mt-5 rounded-full max-sm:rounded-[26px] max-sm:ml-10">
                <div className={`text-center flex-1 mb-3 md:mb-0 ${tab === 'PersonalDevelopment' ? 'bg-amber-500 rounded-full' : ''}`}>
                    <button onClick={() => setTab("PersonalDevelopment")} className="border-none outline-none cursor-pointer bg-transparent block w-full py-5 px-5 hover:bg-aqua">Personal Development</button>
                </div>
                <div className={`text-center flex-1 mb-3 md:mb-0 ${tab === 'SocialImpact' ? 'bg-amber-500 rounded-full' : ''}`}>
                    <button onClick={() => setTab("SocialImpact")} className="border-none outline-none cursor-pointer bg-transparent block w-full py-5 px-5 hover:bg-aqua">Social Impact</button>
                </div>
                <div className={`text-center flex-1 ${tab === 'CuriosityDriven' ? 'bg-amber-500 rounded-full' : ''}`}>
                    <button onClick={() => setTab("CuriosityDriven")} className="border-none outline-none cursor-pointer bg-transparent block w-full py-5 px-5 hover:bg-aqua">Curiosity Driven</button>
                </div>
            </div>
